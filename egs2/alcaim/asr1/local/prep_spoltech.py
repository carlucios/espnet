...
def process_text(text: str) -> str:
    return text.replace(","," ").replace("-"," ").replace("Ã¢","â").replace("Ãº","ú").replace("Ã¡","á").replace("Ã\xad","í").replace("Ãµ","õ").replace("Ãª","ê").replace("Ã´","ô").replace("lazanha","lasanha").replace("Ã£","ã").replace("Ã©","é").replace("Ã§","ç").replace("Ã³","ó").lower()
...
def main(args: List[str]):
    ...
    try:
        ...
        for spkr_name in os.listdir(src):
            ...
            spkid = "spoltech" + spkr_name.split('-')[-1]
            
            for audio in glob(os.path.join(src, spkr_name, '*.wav')):
                basename = os.path.splitext(os.path.basename(audio))[0]
                
                with open(os.path.join(src, spkr_name, basename+'.txt'), encoding='latin1') as f:
