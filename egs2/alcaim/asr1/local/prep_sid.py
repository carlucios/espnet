...
def process_text(text: str) -> str:
    return text.lower().replace(',','').replace('.','').replace('-','').replace(':','').replace(';','').replace('?','').replace("'",'').replace("ü",'u').replace('”','').replace('“','').replace("a4","a quatro").replace("mp3","mp três").replace("90","noventa")
